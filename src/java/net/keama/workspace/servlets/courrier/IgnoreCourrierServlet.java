...
@WebServlet(name="IgnoreCourrierServlet", urlPatterns={"/IgnoreCourrierServlet"})
public class IgnoreCourrierServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Courrier> courriersIgnores = new ArrayList<Courrier>();
        if (courriers != null && !courriers.isEmpty()) {
            for (Courrier c : courriers) {
                if(c.getStatut()!=null && c.getStatut().equals("A ignorer")){
                    courriersIgnores.add(c);
                }
            }
        }
        ...
        request.setAttribute("courriers", courriersIgnores);
        request.setAttribute("employes", employes);

        this.getServletContext().getRequestDispatcher("/WEB-INF/jsp/courrier/ignores.jsp").forward(request, response);
    }
